...
function Mango(obj,option){
    ...
    this.o.isLoop = this.o.isLoop || true;
...
Mango.prototype.touchHandle = function(){
    var that = this;
    that.obj.off('touchstart').on('touchstart',function(e){
        that.startX = e.targetTouches[0].pageX;
        that.startY = e.targetTouches[0].pageY;
        //preventdefault(e);
    });
    that.obj.off('touchmove').on('touchmove',function(e){
        var oEvent = e || event;
        //oEvent.preventDefault();
    });
    that.obj.off('touchend').on('touchend',function(e){
...
Mango.prototype.moveSlid = function(noamin,e){
    var that = this;
    var xPx = 0, yPx = 0;
    //var oEvent = e || event;
...
Mango.prototype.moveCube = function(noamin){
    ...
    if(that.o.dir == 'v' && noamin != true){
        if(that.sub < 0 || moveTo > 0){
            that.ani(oNext,{
                '-webkit-transform':'rotateX(-90deg) translateY('+height+'px) translateZ('+height+'px)',
                'transform':'rotateX(-90deg) translateY('+height+'px) translateZ('+height+'px)'
            },30,function(){
